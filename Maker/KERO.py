...
async def alive(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝐆𝐫𝐨𝐮𝐩 🌸.", url=f"{gr}"),
                InlineKeyboardButton("𝐂𝐡𝐚𝐧𝐧𝐞𝐥 🌸.", url=f"{ch}"),
            ],
            [
                 InlineKeyboardButton(f"{OWNER_NAME}", url=f"https://t.me/{OWNER[0]}")
            ],
            [ 
                 InlineKeyboardButton(text="اضف البوت لـ جروبك 🐾.", url=f"https://t.me/{bot_username}?startgroup=True")
            ]
        ]
    )
