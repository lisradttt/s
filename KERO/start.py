...
from KERO.Data import (get_dev, get_bot_name, set_bot_name, get_logger, get_group, get_channel, get_dev_name, get_groupsr, get_channelsr, get_userbot, set_video_source, set_dev_user)
...
async def gen_bot(client, username, photo):
        ...
        try:
            if os.path.exists(f"thumb{username}.png"):
...
@Client.on_message(filters.command(["مسح"], "") & ~filters.private)
async def deleter(client: Client, message: Message):
...
@Client.on_message(filters.command(["المطور","ميسو","المطور"], ""))
